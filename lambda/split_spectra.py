...
def save_spectra(output_bucket, header, spectra, ts, end_bytes, num_bytes, num_files, final):
  key = "spectra-{0:f}-{1:d}-{2:d}-{3-d}.ms2".format(ts, num_files, end_bytes, num_bytes, suffix)
...
def split_spectra(bucket_name, key, batch_size, chunk_size):
  ...
  while start_byte < num_bytes:
    ...
    if len(parts) > 1:
      ...
      while len(spectra) >= batch_size:
        save_spectra(output_bucket, header, "S ".join(spectra[:batch_size]), ts, num_files, False)
  ...
  save_spectra(output_bucket, header, "S ".join(spectra), ts, num_files, True)
